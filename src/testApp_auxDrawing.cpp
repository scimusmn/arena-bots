...
void testApp::drawMenuBar(int & yPos, int & hgt)
{
  ...
  ofSetShadowDarkness(.25);
  if(sets.size()){
    ofButton & last=sets(sets.size()-1);
    ofShade((undoBut.x+last.x+last.w)/2, yPos, 10, hgt, OF_LEFT);
    ofShade((undoBut.x+last.x+last.w)/2, yPos, 10, hgt, OF_RIGHT,false);
  }
  
  ofShade((clearBut.x+redoBut.x+redoBut.w)/2, yPos, 10, hgt, OF_LEFT);
  ofShade((clearBut.x+redoBut.x+redoBut.w)/2, yPos, 10, hgt, OF_RIGHT,false);
}
...
void testApp::drawAttract()
{
  ...
  if(sets.size()>1)
    ofFlat(),ofRoundedRect(ofGetWidth()/(sets.size()+1)-sets(0).w/2-20, k.y-20, ((sets(sets.size()-1).x+sets(sets.size()-1).w)-sets(0).x)+40, k.height+sets(0).h+40, 10);
  else{
    timeOut.set(180);
  }
